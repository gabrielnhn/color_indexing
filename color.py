...
for scene_file in os.listdir(scenes_dir) + os.listdir(objects_dir):
    if scene_file in os.listdir(scenes_dir):
        scene_file = os.path.join(scenes_dir, scene_file)
    else:
        scene_file = os.path.join(objects_dir, scene_file)
    ...
    for obj in objects:
        ...
        dst = cv.calcBackProject([hsvt],[0,1],obj["hist"],[0,256,0,256],1)
        ...
        _,thresh = cv.threshold(dst,30,256,cv.THRESH_BINARY)
        ...
        # x,y,w,h = box
        # total_white = cv.countNonZero(thresh[y:y+h,x:x+w])
        ...
        if not box_is_empty:
            ...
            cv.drawContours(highlighted, contours, -1, color, 3)
